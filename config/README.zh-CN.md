...
`codex.json` 是提交到仓库、所有 design 共用的 Teacher/Student 模型、推理强度、重试和超时策略。worker home 在每个 campaign 的 `state_root` 下创建。`credentials/goalevolve_codex.env` 仅保存本地 API key 与 provider 连接信息，且被 Git 忽略。机器相关默认值使用 `GOALEVOLVE_OPENROAD_SEED`、`GOALEVOLVE_BENCHMARK_ROOT`，或写入受审的相对路径 profile。
