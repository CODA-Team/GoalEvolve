...
existing exact declarator comparison.  Preserve identifiers, type tokens,
template contents, qualifiers, and parameter order.  The resolver must still
return `ambiguous` for a bare overloaded name and must not match a different
parameter list.
...
in punctuation-adjacent whitespace.  Verify the test is red before the change,
green after it, and retain the existing overload ambiguity test as the guard
against an unsafe relaxation.
