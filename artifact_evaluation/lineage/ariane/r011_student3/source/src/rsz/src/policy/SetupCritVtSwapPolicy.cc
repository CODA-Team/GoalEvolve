...
bool SetupCritVtSwapPolicy::swapVTCritCells(int& num_viols)
{
  ...
  if (readCritFlagEnv("RSZ_CRIT_VT_LEGACY_BATCH", false)) {
  ...
  for (const std::pair<sta::Instance* const, float>& crit_inst_slack :
    ...
    const float leakage_delta
        = leakageOf(resizer_, best_cell) - leakageOf(resizer_, current_cell);
    if (leakage_delta >= 0.0f) {
      continue;
    }
    const float score
  ...
  int committed = 0;
  float committed_leakage_delta = 0.0f;
  if (guard_enabled) {
  ...
  for (const CritVtCandidate& scored : candidates) {
    ...
    if (result.accepted) {
      changed = true;
      ++committed;
      committed_leakage_delta += scored.leakage_delta;
      debugPrint(logger_,
                 RSZ,
                 "swap_crit_vt",
                 1,
  ...
  if (changed) {
    ...
    if (guarded_accept) {
      ...
      logger_->report("METRIC|crit_vt_power_moves|{}", committed);
      logger_->report("METRIC|crit_vt_leakage_delta|{:.6g}",
                      committed_leakage_delta);
      debugPrint(logger_,
                 RSZ,
                 "swap_crit_vt",
                 1,
