...
constexpr int kBufferRemovalMaxFanout = 4;
...
struct UnbufferSelectionContext {
  ...
  UnbufferSelectionContext(Resizer &resizer, MoveCommitter &committer,
                           const Target &target, float setup_slack_margin)
      : resizer(resizer), committer(committer), target(target),
...
bool passesMoveConflictGuard(const UnbufferSelectionContext &ctx) {
  ...
  debugPrint(ctx.resizer.logger(), RSZ, "unbuffer_move", 4,
             "buffer {} is not removed because {}",
             ctx.resizer.dbNetwork()->name(ctx.drvr), reason);
  return false;
}
...
bool passesFanoutGuard(const UnbufferSelectionContext &ctx) {
  ...
  float limit;
  float slack;
  ctx.resizer.sta()->checkFanout(ctx.prev_drvr_pin, ctx.scene->mode(),
                                 ctx.resizer.maxAnalysisMode(), fanout, limit,
                                 slack);
  ...
  if (limit > 0.0) {
    ...
    debugPrint(
        ctx.resizer.logger(), RSZ, "unbuffer_move", 2,
        "buffer {} is not removed because of max fanout limit of {} at {}",
        ctx.resizer.dbNetwork()->name(ctx.drvr), limit,
  ...
  debugPrint(
      ...
      ctx.resizer.dbNetwork()->name(ctx.drvr), kBufferRemovalMaxFanout,
      ctx.resizer.network()->pathName(ctx.prev_drvr_pin));
  return false;
}
...
bool passesCapGuard(UnbufferSelectionContext &ctx) {
  ...
  ctx.resizer.sta()->checkCapacitance(
      ctx.prev_drvr_pin, ctx.resizer.sta()->scenes(),
      ctx.resizer.maxAnalysisMode(), cap, max_cap, cap_slack, tr, corner);
  ...
  debugPrint(ctx.resizer.logger(), RSZ, "unbuffer_move", 2,
             "buffer {} is not removed because of max cap limit of {} at {}",
             ctx.resizer.dbNetwork()->name(ctx.drvr), max_cap,
             ctx.resizer.network()->pathName(ctx.prev_drvr_pin));
  return false;
}
