...
void RepairPowerPolicy::loadConfig() {
  ...
  if (cfg_.phase == "early_forced_reclaim") {
    ...
    if (!envSet("RSZ_REPAIR_POWER_MAX_MOVES") && config_.power_max_moves <= 0) {
      const int proportional_moves
          = std::max(800,
                     static_cast<int>(eligible * cfg_.proportion * 0.36f));
      cfg_.max_moves = std::min(proportional_moves, 4200);
      cfg_.budgeted_late_tail = proportional_moves > 4200;
    }
    if (!envSet("RSZ_REPAIR_POWER_MAX_TARGETS")) {
      cfg_.max_targets = std::min(
...
RepairPowerPolicy::Metrics RepairPowerPolicy::collectMetrics() const {
  ...
  metrics.leakage = collectLeakage();
...
void RepairPowerPolicy::iterateEarlyForcedReclaim() {
  ...
  while (!moveLimitReached() && !trialLimitReached()) {
    ...
    orderComparableCandidatesByTimingStability(candidates);
...
void RepairPowerPolicy::iterateLateLeakageRecovery() {
  constexpr int tail_retained_limit = 256;
  constexpr int tail_examined_limit = 512;
  constexpr int tail2_retained_limit = 256;
  constexpr int tail2_examined_limit = 512;
  const int retained_move_cap = cfg_.max_moves;
  bool changed = false;
  Metrics current = collectMetrics();
  int tail_examined = 0;
  int tail_retained = 0;
  int tail2_examined = 0;
  int tail2_retained = 0;
  const auto moveLimitReached = [&]() {
    if (committed_ < retained_move_cap) {
      return false;
    }
    if (!cfg_.budgeted_late_tail) {
      return true;
    }
    if (tail_retained < tail_retained_limit) {
      return tail_examined >= tail_examined_limit;
    }
    return tail2_retained >= tail2_retained_limit
           || tail2_examined >= tail2_examined_limit;
  };
  const auto trialLimitReached = [&]() {
    if (trials_ < cfg_.trial_limit) {
      return false;
    ...
    const int tail_trial_limit
        = tail_retained < tail_retained_limit
              ? tail_examined_limit
              : tail_examined_limit + tail2_examined_limit;
    return trials_ >= cfg_.trial_limit + tail_trial_limit;
  };
  while (!moveLimitReached() && !trialLimitReached()) {
    std::vector<Candidate> candidates;
    ...
    for (size_t i = 0; i < candidates.size();) {
      ...
      const int active_window_size = cfg_.adaptive_timing_window
                                         ? adaptive_window_size_
                                         : cfg_.timing_update_interval;
      if (committed_ < retained_move_cap && active_window_size > 1 &&
          !candidateNeedsImmediateTimingGuard(candidate)) {
        std::vector<Candidate> window;
        std::unordered_set<sta::Instance *> window_insts;
        size_t j = i;
      ...
      const bool tail_candidate = committed_ >= retained_move_cap;
      const bool tail2_candidate
          = tail_candidate && tail_retained >= tail_retained_limit;
      if (tail2_candidate) {
        ++tail2_examined;
        logger_->report("METRIC|repair_power_late_tail2_examined|1");
      } else if (tail_candidate) {
        ++tail_examined;
        logger_->report("METRIC|repair_power_late_tail_examined|1");
      }
      ++trials_;
      if (tryCommitLateCandidate(candidate, current)) {
        ...
        if (tail2_candidate) {
          ++tail2_retained;
          logger_->report("METRIC|repair_power_late_tail2_retained|1");
        } else if (tail_candidate) {
          ++tail_retained;
          logger_->report("METRIC|repair_power_late_tail_retained|1");
        }
      } else {
        ++rejected_;
      }
...
bool RepairPowerPolicy::tryCommitGuardedSwap(const Candidate &candidate,
  ...
  const std::string inst_name = instName(candidate.target.inst);
  const double leakage_before = collectLeakage();
  resizer_.journalBegin();
  if (!resizer_.replaceCell(candidate.target.inst, candidate.replacement)) {
    resizer_.journalRestore();
    recordReject("apply");
  ...
  logger_->report("METRIC|jpeg_mid_power_examined|1");
  const double leakage_after = collectLeakage();
  if (leakage_before - leakage_after <= 0.0) {
    resizer_.journalRestore();
    recordReject("mid_measured_leakage");
    return false;
  }

  resizer_.journalEndNoTimingUpdate();
  logger_->report("METRIC|jpeg_mid_power_retained|1");
  current = after;
  accountAcceptedCandidate(candidate);
...
bool RepairPowerPolicy::tryCommitGuardedSwapWindow(
  ...
  if (leakage_before - leakage_after <= 0.0) {
    resizer_.journalRestore();
    recordReject("mid_window_measured_leakage");
    return false;
  }
...
void RepairPowerPolicy::orderComparableCandidatesByTimingStability(
  ...
  while (begin < candidates.size()) {
    ...
    const auto [min_slack, max_slack] = std::minmax_element(
        candidates.begin() + begin, candidates.begin() + end,
        [](const Candidate &lhs, const Candidate &rhs) {
          return lhs.target.slack < rhs.target.slack;
        });
    if (end - begin > 1 && min_slack->target.slack < max_slack->target.slack) {
      logger_->report("METRIC|repair_power_stability_examined|{}", end - begin);
      for (size_t index = begin; index < end; ++index) {
        candidates[index].stability_ordered = true;
      }
      std::stable_sort(candidates.begin() + begin, candidates.begin() + end,
                       [](const Candidate &lhs, const Candidate &rhs) {
                         return lhs.target.slack > rhs.target.slack;
                       });
    }
...
void RepairPowerPolicy::accountAcceptedCandidate(const Candidate &candidate) {
  if (candidate.stability_ordered) {
    logger_->report("METRIC|repair_power_stability_retained|1");
  }
  if (cfg_.late_second_pass) {
...
double RepairPowerPolicy::collectLeakage() const {
  double leakage = 0.0;
  sta::LeafInstanceIterator *iter = network_->leafInstanceIterator();
  while (iter->hasNext()) {
    sta::Instance *inst = iter->next();
