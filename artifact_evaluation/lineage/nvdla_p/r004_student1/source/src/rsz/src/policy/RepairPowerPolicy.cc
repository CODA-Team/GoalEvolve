...
void RepairPowerPolicy::iterateEarlyForcedReclaim() {
  ...
  int tail_examined = 0;
  int tail_retained = 0;
  std::vector<Candidate> deferred_halo_candidates;
  std::unordered_set<sta::Instance *> deferred_halo_instances;
  ...
  const auto deferHaloCandidate = [&](const Candidate &candidate,
                                      const bool halo_excluded) {
    if (!halo_excluded ||
        !deferred_halo_instances.insert(candidate.target.inst).second) {
      return false;
    }
    deferred_halo_candidates.push_back(candidate);
    return true;
  };
  while (!moveLimitReached() && !trialLimitReached()) {
    ...
    for (size_t i = 0; i < candidates.size();) {
      ...
      if (touched_.contains(candidate.target.inst) ||
          deferred_halo_instances.contains(candidate.target.inst)) {
        ++i;
        continue;
      }
      ...
      if (active_window_size > 1 &&
        ...
        while (j < candidates.size() &&
          ...
          if (touched_.contains(window_candidate.target.inst) ||
              deferred_halo_instances.contains(window_candidate.target.inst) ||
        ...
        if (tryCommitCandidateWindow(window, current)) {
          ...
          for (const Candidate &window_candidate : window) {
            ...
            bool halo_excluded = false;
            if (tryCommitCandidate(window_candidate, current, false,
                                   &halo_excluded)) {
              changed = true;
              ++committed_;
              ++accepted_this_batch;
              ...
              ++rejected_;
              ++split_rejects;
              if (!deferHaloCandidate(window_candidate, halo_excluded)) {
                touched_.insert(window_candidate.target.inst);
              }
            }
          }
        ...
        for (const Candidate &window_candidate : window) {
          if (!deferred_halo_instances.contains(window_candidate.target.inst)) {
            touched_.insert(window_candidate.target.inst);
          }
        }
        i = j;
      ...
      bool halo_excluded = false;
      if (tryCommitCandidate(candidate, current, false, &halo_excluded)) {
        changed = true;
        ++committed_;
        ++accepted_this_batch;
        if (tail_candidate) {
        ...
        ++rejected_;
        if (!deferHaloCandidate(candidate, halo_excluded)) {
          touched_.insert(candidate.target.inst);
        }
      }
      ++i;
  ...
  for (const Candidate &candidate : deferred_halo_candidates) {
    if (committed_ >= retained_move_cap || trials_ >= cfg_.trial_limit) {
      break;
    }
    if (touched_.contains(candidate.target.inst) ||
        !isStrictPositiveLeakageHaloCandidate(candidate)) {
      continue;
    }
    ++trials_;
    if (excludeCriticalConeReversion(candidate)) {
    ...
    if (tryCommitCandidate(candidate, current, true)) {
      changed = true;
      ++committed_;
    } else {
      ++rejected_;
...
bool RepairPowerPolicy::tryCommitCandidate(
    const Candidate &candidate,
    Metrics &current,
    const bool integrated_halo_reconsideration,
    bool *halo_excluded) {
  if (halo_excluded != nullptr) {
    *halo_excluded = false;
  }
  ...
  if (candidate.kind == MoveKind::kSizeDownAndPowerVtSwap) {
    ...
    if (!integrated_halo_reconsideration &&
        excludeCompoundVtCriticalHalo(candidate)) {
    ...
    if (!integrated_halo_reconsideration &&
        excludeCompoundVtCriticalFaninHalo(candidate)) {
  ...
  if (candidate.kind == MoveKind::kRemoveBuffer) {
    ...
    if (!integrated_halo_reconsideration &&
        excludePureVtCriticalHalo(candidate)) {
      resizer_.journalRestore();
...
bool RepairPowerPolicy::isStrictPositiveLeakageHaloCandidate(
    const Candidate &candidate) const {
  if (cfg_.phase != "early_forced_reclaim" ||
  ...
  return compound_vt_critical_halo_.contains(candidate.target.inst) ||
         compound_vt_critical_fanin_halo_.contains(candidate.target.inst);
}
