...
static constexpr float kSupplementalTnsExpansionCapRatio = 0.080f;
    ...
    if (!initializeLastGaspRepair(last_gasp_state, violating_ends)) {
      ...
      if (logger_->debugCheck(RSZ, "repair_setup", 1)) {
        ...
        debugPrint(logger_, RSZ, "repair_setup", 1,
                   "LAST_GASP{} Phase complete. WNS: {}, TNS: {}", phase_marker,
...
bool SetupLastGaspPolicy::initializeLastGaspRepair(
  ...
  last_gasp_state.phase_baseline_tns = sta_->totalNegativeSlack(max_);
  sta::Vertex *phase_baseline_worst = nullptr;
  sta_->worstSlack(max_, last_gasp_state.phase_baseline_worst_slack,
                   phase_baseline_worst);
  ...
  const float curr_tns = last_gasp_state.phase_baseline_tns;
  if (sta::fuzzyGreaterEqual(curr_tns, 0)) {
    debugPrint(logger_, RSZ, "repair_setup", 1,
               "LAST_GASP{} Phase: TNS is {}, exiting",
               last_gasp_state.phase_marker, delayAsString(curr_tns, 1, sta_));
    return false;
  }
  ...
  debugPrint(logger_, RSZ, "repair_setup", 1,
             ...
             last_gasp_state.phase_marker, last_gasp_state.max_end_count);
  printProgress(last_gasp_state.opto_iteration, false,
                last_gasp_state.phase_marker);
  return true;
}
...
bool SetupLastGaspPolicy::beginLastGaspEndpoint(
  ...
  if (!beginJournaledEndpointSearch(
          end_original_slack, last_gasp_state.max_end_count,
          last_gasp_state.end_index, endpoint_state)) {
    debugPrint(logger_, RSZ, "repair_setup", 1,
  ...
  debugPrint(logger_, RSZ, "repair_setup", 1,
             ...
             "{}, WNS = {}",
             last_gasp_state.phase_marker, endpoint_state.end->name(network_),
             last_gasp_state.end_index, last_gasp_state.max_end_count,
...
bool SetupLastGaspPolicy::lastGaspImproved(const sta::Slack worst_slack,
  ...
  const float tns_expand_ratio =
      std::max(0.0f, std::abs(curr_tns) - std::abs(prev_tns)) /
      std::max(1.0f, std::abs(prev_tns));
  return sta::fuzzyGreaterEqual(worst_slack, prev_worst_slack) &&
         sta::fuzzyGreaterEqual(curr_tns, prev_tns) &&
         tns_expand_ratio <= kSupplementalTnsExpansionCapRatio;
}

bool SetupLastGaspPolicy::advanceLastGaspProgress(
    SetupLegacyBase::EndpointRepairState &endpoint_state,
    SetupLastGaspPolicy::LastGaspState &last_gasp_state, const float curr_tns) {
  if (!lastGaspImproved(endpoint_state.worst_slack, curr_tns,
                        last_gasp_state.prev_worst_slack,
                        last_gasp_state.prev_tns)) {
    debugPrint(
        logger_, RSZ, "repair_setup", 2,
        "LAST_GASP{} Phase: Move rejected for endpoint {} pass {} "
        "because WNS worsened {} -> {} and TNS worsened {} -> {}",
        last_gasp_state.phase_marker, last_gasp_state.end_index,
        endpoint_state.pass,
  ...
  logger_->report("METRIC|timing_recovery_last_gasp_examined|1");
  if (!sta::fuzzyGreater(curr_tns, last_gasp_state.phase_baseline_tns) ||
      !sta::fuzzyGreaterEqual(endpoint_state.worst_slack,
                              last_gasp_state.phase_baseline_worst_slack)) {
    debugPrint(
        logger_, RSZ, "repair_setup", 2,
        "LAST_GASP{} Phase: Move rolled back because phase-baseline TNS/WNS "
        "gate failed: TNS {} -> {}, WNS {} -> {}",
        last_gasp_state.phase_marker,
        delayAsString(last_gasp_state.phase_baseline_tns, 1, sta_),
        delayAsString(curr_tns, 1, sta_),
        delayAsString(last_gasp_state.phase_baseline_worst_slack, kDelayDigits,
                      sta_),
        delayAsString(endpoint_state.worst_slack, kDelayDigits, sta_));
    restoreEndpointState(endpoint_state);
    return false;
  ...
  if (sta::fuzzyGreaterEqual(endpoint_state.end_slack,
    ...
    logger_->report("METRIC|timing_recovery_last_gasp_retained|1");
    return false;
  }
...
void SetupLastGaspPolicy::repairLastGaspEndpoint(
  ...
  while (endpoint_state.pass <= max_last_gasp_passes_) {
    ...
    if (terminateProgress(
            last_gasp_state.opto_iteration, last_gasp_state.initial_tns,
            last_gasp_state.prev_tns, last_gasp_state.fix_rate_threshold,
            last_gasp_state.end_index, last_gasp_state.max_end_count,
            "LAST_GASP", last_gasp_state.phase_marker)) {
...
void SetupLastGaspPolicy::runLastGaspLoop(
  ...
  for (const auto &end_original_slack : violating_ends) {
    ...
    if (!beginLastGaspEndpoint(end_original_slack, last_gasp_state,
                               endpoint_state)) {
      break;
    }
    ...
    if (config_.verbose || last_gasp_state.opto_iteration == 1) {
      printProgress(last_gasp_state.opto_iteration, true,
                    last_gasp_state.phase_marker);
    }
    if (shouldStopLastGasp(last_gasp_state)) {
      debugPrint(logger_, RSZ, "repair_setup", 1,
