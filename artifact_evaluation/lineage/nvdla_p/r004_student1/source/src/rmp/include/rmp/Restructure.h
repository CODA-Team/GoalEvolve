...
class Restructure
{
 ...
 private:
  ...
  bool safeRestructureInst(odb::dbInst* inst) const;
  bool hasSingleOutput(sta::LibertyCell* cell) const;
  bool isSupportedMultiOutputCell(sta::LibertyCell* cell) const;
  bool touchesSequentialPin(odb::dbInst* inst) const;
  bool touchesClockNetwork(odb::dbInst* inst) const;
  bool isClockLikeCell(odb::dbInst* inst) const;
  std::set<std::string> cloudBoundaryOutputNetNames(
      const std::set<odb::dbInst*>& cloud) const;
  int cloudBoundaryOutputCount(const std::set<odb::dbInst*>& cloud) const;
  bool runABC();
  ...
  std::vector<std::set<odb::dbInst*>> path_inst_clouds_;
  std::vector<std::set<std::string>> path_inst_cloud_names_;
  std::vector<std::string> path_inst_cloud_target_net_names_;
  std::string path_inst_target_net_name_;
