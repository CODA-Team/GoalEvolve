...
{
  for (const MoveType candidate : sequence) {
    if (candidate == move) {
      return true;
    }
  }
    ...
    const OptimizerRunConfig& config)
{
  ...
  const bool vt_swap_allowed_by_sequence =
      config.sequence.empty() || containsMove(config.sequence, MoveType::kVtSwap);
  if (config.skip_vt_swap || config.skip_crit_vt_swap
      || !vt_swap_allowed_by_sequence) {
  ...
  if (needs_implicit_crit_vt_swap
    ...
    // requested CRIT_VT_SWAP explicitly.  If the caller provided a custom move
    // sequence without vt_swap, honor that lower-power schedule and do not
    // append an implicit VT phase.
    ...
    RepairSetupContext& setup_context)
{
  ...
  if (phase_name == "MEASURED_CRIT_PATH") {
    return std::make_unique<MeasuredCriticalPathPolicy>(
        resizer_, committer_, setup_context, config_);
  }
  if (phase_name == "POWER_RECOVERY_PLUS") {
    return std::make_unique<PowerRecoveryPlusPolicy>(
        resizer_, committer_, setup_context, config_);
  }
  if (phase_name == "REPAIR_POWER") {
    return std::make_unique<RepairPowerPolicy>(
        resizer_, committer_, setup_context, config_);
  }
  if (phase_name == "IMPLICIT_REPAIR_POWER") {
    return std::make_unique<PowerRecoveryPlusPolicy>(
        resizer_, committer_, setup_context, config_);
  }
  // Only public phase names are listed; experimental top-level tokens
  // (LEGACY_MT, MT1, MEASURED_VT_SWAP, MEASURED_CRIT_PATH) are accepted but
  // undocumented.
  resizer_.logger()->error(
      ...
      "LAST_GASP, CRIT_VT_SWAP, REROUTE, POWER_RECOVERY_PLUS, REPAIR_POWER, "
      "MEASURED_CRIT_PATH",
      phase_name);
  return nullptr;
}
...
bool Optimizer::run()
{
  ...
  std::vector<std::string> phase_names
      = appendImplicitCritVtSwapForLegacyPhases(requested_phase_names, config_);
  if (config_.phases.empty()
      && containsPhase(phase_names, "CRIT_VT_SWAP")) {
    phase_names.emplace_back("IMPLICIT_REPAIR_POWER");
    config_.power_phase = "late_leakage_recovery";
    if (config_.power_max_tns_expand_ratio <= 0.0) {
      config_.power_max_tns_expand_ratio = 0.02;
    }
    if (config_.power_max_wns_drop <= 0.0) {
      config_.power_max_wns_drop = 1.0e-11;
    }
  }
  if (phase_names.empty()) {
    resizer_.logger()->error(
        ...
        "WNS_PATH, WNS_CONE, TNS, ENDPOINT_FANIN, STARTPOINT_FANOUT, "
        "LAST_GASP, CRIT_VT_SWAP, REROUTE, POWER_RECOVERY_PLUS, "
        "MEASURED_CRIT_PATH");
  }
