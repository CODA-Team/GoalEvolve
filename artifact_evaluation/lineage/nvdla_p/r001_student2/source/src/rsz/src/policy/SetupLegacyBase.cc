...
bool rankPathDriversByTotalDelay()
{
  const char* value = std::getenv("RSZ_RANK_PATH_BY_TOTAL_DELAY");
  return value != nullptr && string(value) != "0";
}

bool rankWeakSizeupDrivers()
{
  const char* value = std::getenv("RSZ_RANK_WEAK_SIZEUP_DRIVERS");
  return value != nullptr && string(value) != "0";
}

float weakSizeupDriverBoost(const sta::LibertyCell* cell)
{
  if (cell == nullptr || !rankWeakSizeupDrivers()) {
    return 0.0f;
  }

  const string name = cell->name();
  float boost = 0.0f;
  if (name.find("xp33") != string::npos
      || name.find("XP33") != string::npos) {
    boost = std::max(boost, 1.5e-11f);
  } else if (name.find("xp5") != string::npos
             || name.find("XP5") != string::npos) {
    boost = std::max(boost, 6.0e-12f);
  }
  if (boost > 0.0f
      && (name.find("_R") != string::npos
          || name.find("_RVT") != string::npos)) {
    boost += 4.0e-12f;
  }
  return boost;
}
    ...
    const int lib_ap) const
{
  ...
  // Rank by legacy load-only delay by default. Add a narrow weak-cell boost so
  // tiny post-reclaim drivers on violating paths compete for structural
  // size-up budget before the run spends equivalent effort on VT-only fixes.
  const bool rank_by_total_delay = rankPathDriversByTotalDelay();
  for (int i = start_index; i < path_length; i++) {
    ...
    if (i > 0 && path_vertex->isDriver(network_)
      ...
      const sta::Delay stage_delay =
          graph_->arcDelay(
              prev_edge, prev_arc, corner->dcalcAnalysisPtIndex(max_));
      sta::LibertyPort* drvr_port = network_->libertyPort(path_pin);
      sta::LibertyCell* drvr_cell
          = drvr_port != nullptr ? drvr_port->libertyCell() : nullptr;
      load_delays.emplace_back(
          i,
          (rank_by_total_delay ? stage_delay : load_delay)
              + weakSizeupDriverBoost(drvr_cell));
    }
  }
