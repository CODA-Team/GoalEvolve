...
void FastRouteCore::updateSlacks(float percentage)
{
  ...
  const float setup_worst_slack = sta_->worstSlack(sta::MinMax::max());
  if (!std::isfinite(setup_worst_slack)) {
    return;
  }
  ...
  const int selected_net_count = std::ceil(res_aware_list.size() * percentage);
  for (int i = 0; i < selected_net_count; i++) {
  ...
  if (marked_net_count > 0) {
    logger_->report("METRIC|resistance_aware_nets|{}", marked_net_count);
  }
}
