...
void OptimizationPolicy::loadPolicyEnvars()
{
  ...
  policy_config_.timing_power_aware_scoring
      = utl::readEnvarInt("RSZ_TIMING_POWER_AWARE", 1) > 0;
  policy_config_.timing_sizeup_power_penalty
      = readEnvFloat("RSZ_TIMING_SIZEUP_POWER_PENALTY", 0.0f);
  policy_config_.timing_vt_power_penalty
      = readEnvFloat("RSZ_TIMING_VT_POWER_PENALTY", 0.0f);
  policy_config_.timing_vt_growth_slack
      = readEnvFloat("RSZ_TIMING_VT_GROWTH_SLACK", 1.0f);
}
