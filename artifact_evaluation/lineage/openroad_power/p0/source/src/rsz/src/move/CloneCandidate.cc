...
Estimate CloneCandidate::estimate()
{
  ...
  const int moved_count = static_cast<int>(moved_loads_.size());
  if (moved_count < 2 || fanout < 4 || target_.slack > -8.0e-11f) {
  ...
  const float timing_gain
      = (0.7e-11f + 0.55e-11f * criticality) * relief_ratio * fanout_scale;

  const float original_leakage = leakageOf(resizer_, original_cell_);
  const float clone_leakage = leakageOf(resizer_, clone_cell_);
  const float leakage_cost
      = 2.0e-12f * clone_leakage / positiveOrOne(original_leakage);
  const float area_cost
      = 0.45e-12f * (clone_cell_ != nullptr ? clone_cell_->area() : 0.0f)
        / positiveOrOne(original_cell_ != nullptr ? original_cell_->area()
                                                  : 0.0f);
  const float score = timing_gain - leakage_cost - area_cost;
