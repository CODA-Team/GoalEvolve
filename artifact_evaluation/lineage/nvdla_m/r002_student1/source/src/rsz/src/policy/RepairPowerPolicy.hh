...
private:
  ...
  struct Config {
    ...
    double absolute_tns_ceiling{0.0};
  ...
  bool tryCommitCandidate(const Candidate &candidate, Metrics &current,
                          bool integrated_halo_reconsideration = false);
  ...
  bool withinLateLeakageBudget(const Metrics &before, const Metrics &after,
                               const Metrics &baseline) const;
  bool withinLateTimingBudget(const Metrics &before, const Metrics &after,
                              const Metrics &baseline) const;
  bool exceedsStageTnsCeiling(const Metrics &metrics) const;
  ...
  bool canReconsiderIntegratedHaloCandidate(const Candidate &candidate) const;
  ...
  std::unordered_set<sta::Instance *> compound_vt_critical_halo_;
  std::unordered_set<sta::Instance *> compound_vt_critical_fanin_halo_;
  Candidate integrated_halo_candidate_;
  bool has_integrated_halo_candidate_{false};
