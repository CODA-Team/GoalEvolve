...
struct Target
{
  ...
  const sta::Pin* endpoint_pin{nullptr};
  int path_index{-1};
...
struct OptimizerRunConfig
{
  ...
  // True only for a public repair_power command invocation. repair_timing
  // callers keep the false default, including when they set power_phase.
  bool repair_power_command{false};

  // repair_power command phase context.
  std::string power_phase;
  float power_proportion{1.0f};
  int power_max_moves{0};
  double power_max_tns_expand_ratio{0.0};
  double power_max_wns_drop{0.0};
};
...
struct OptimizationPolicyConfig
{
  ...
  // Timing repair power guard. These knobs bias setup repair away from
  // leakage-expensive VT swaps and over-strong sizeups when a lower-power move
  // has comparable delay payoff.
  bool timing_power_aware_scoring{true};
  float timing_sizeup_power_penalty{2.0e-12f};
  float timing_vt_power_penalty{7.0e-12f};
  float timing_vt_growth_slack{-1.0e-9f};
};
