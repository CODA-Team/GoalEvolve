...
bool RepairPowerPolicy::start() {
  ...
  has_integrated_halo_candidate_ = false;
  touched_.clear();
  ...
  logger_->info(RSZ, 2320,
                ...
                "max_tns_expand_ratio={:.3g}|stage_tns_ceiling={}|max_wns_drop={}|"
                "vt_swap={}|skip_seq={}",
                ...
                sta::delayAsString(cfg_.absolute_tns_ceiling, 3, sta_),
...
void RepairPowerPolicy::loadConfig() {
  ...
  cfg_.absolute_tns_ceiling = readDoubleEnv(
      "RSZ_POWER_STAGE_TNS_CEILING_S", cfg_.absolute_tns_ceiling);
  if (!std::isfinite(cfg_.absolute_tns_ceiling)
      || cfg_.absolute_tns_ceiling <= 0.0) {
    cfg_.absolute_tns_ceiling = 0.0;
  }
  cfg_.max_wns_drop =
...
void RepairPowerPolicy::iterateEarlyForcedReclaim() {
  ...
  if (has_integrated_halo_candidate_ && committed_ < cfg_.max_moves &&
      trials_ < cfg_.trial_limit) {
    ++trials_;
    logger_->report("METRIC|repair_power_integrated_halo_examined|1");
    if (tryCommitCandidate(integrated_halo_candidate_, current, true)) {
      changed = true;
      ++committed_;
    } else {
      ++rejected_;
    }
    touched_.insert(integrated_halo_candidate_.target.inst);
  }
...
bool RepairPowerPolicy::tryCommitCandidate(const Candidate &candidate,
                                           Metrics &current,
                                           bool integrated_halo_reconsideration) {
  ...
  if (candidate.kind == MoveKind::kRemoveBuffer) {
    ...
    if (!integrated_halo_reconsideration &&
      ...
      if (canReconsiderIntegratedHaloCandidate(candidate)) {
        integrated_halo_candidate_ = candidate;
        has_integrated_halo_candidate_ = true;
      }
      resizer_.journalRestore();
  ...
  if (integrated_halo_reconsideration || cfg_.full_metrics_interval <= 1 ||
...
bool RepairPowerPolicy::withinPhaseBudget(const Metrics &before,
  ...
  if (exceedsStageTnsCeiling(after)) {
    return false;
  }
...
bool RepairPowerPolicy::exceedsStageTnsCeiling(
    const Metrics &metrics) const {
  return cfg_.absolute_tns_ceiling > 0.0
         && std::abs(static_cast<double>(metrics.tns))
                > cfg_.absolute_tns_ceiling;
}
...
bool RepairPowerPolicy::canReconsiderIntegratedHaloCandidate(
    const Candidate &candidate) const {
  return config_.repair_power_command &&
         cfg_.phase == "early_forced_reclaim" &&
         !has_integrated_halo_candidate_ &&
         candidate.kind == MoveKind::kPowerVtSwap &&
         candidate.leakage_gain > 0.0 && candidate.target.cell != nullptr &&
         candidate.replacement != nullptr &&
         isStrictPowerDirectionVtSwap(candidate.target.cell,
                                      candidate.replacement);
}
