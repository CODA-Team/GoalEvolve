...
using PinSlack = std::pair<const sta::Pin *, sta::Slack>;
using VertexSlack = std::pair<sta::Vertex *, sta::Slack>;

constexpr float kGlobalSetupPathSlackTie = 1.0e-12f;

int wireFanout(sta::Vertex *vertex, sta::Graph *graph) {
  if (vertex == nullptr || graph == nullptr) {
    return 0;
  }
  int fanout = 0;
...
bool betterGlobalSetupEndpoint(const sta::Slack path_slack,
                               const sta::Slack endpoint_slack,
                               const sta::Slack best_slack,
                               const sta::Slack best_endpoint_slack) {
  return path_slack < best_slack - kGlobalSetupPathSlackTie ||
         (std::abs(path_slack - best_slack) <= kGlobalSetupPathSlackTie &&
          endpoint_slack < best_endpoint_slack);
}

bool slackLess(const PinSlack &lhs, const PinSlack &rhs) {
  return lhs.second < rhs.second;
}

bool pathEndpointLess(const VertexSlack &lhs, const VertexSlack &rhs) {
  return sta::fuzzyLess(lhs.second, rhs.second);
}
...
float weakDriverHeuristicBonus(sta::Network *network, const sta::Pin *pin) {
  if (!envEnabled("RSZ_HEURISTIC_WEAK_DRIVER_PRIORITY", true)) {
    return 0.0f;
  }
  sta::LibertyPort *port = network->libertyPort(pin);
  sta::LibertyCell *cell = port != nullptr ? port->libertyCell() : nullptr;
  if (cell == nullptr) {
    return 0.0f;
  }

  const std::string name = cell->name();
  if (name.find("xp33") != std::string::npos
      || name.find("XP33") != std::string::npos) {
    return envFloat("RSZ_HEURISTIC_XP33_BONUS", 2.5e-11f);
  }
  if (name.find("xp5") != std::string::npos
      || name.find("XP5") != std::string::npos) {
    return envFloat("RSZ_HEURISTIC_XP5_BONUS", 1.2e-11f);
  }
  return 0.0f;
}

float weakDriverLoadDelayBonus(sta::Network *network, const sta::Pin *pin) {
  if (!envEnabled("RSZ_LOAD_DELAY_WEAK_DRIVER_PRIORITY", true)) {
    return 0.0f;
  }
  sta::LibertyPort *port = network->libertyPort(pin);
  sta::LibertyCell *cell = port != nullptr ? port->libertyCell() : nullptr;
  ...
  if (name.find("xp33") != std::string::npos
      || name.find("XP33") != std::string::npos) {
    bonus = envFloat("RSZ_LOAD_DELAY_XP33_BONUS", 3.5e-11f);
  } else if (name.find("xp5") != std::string::npos
             || name.find("XP5") != std::string::npos) {
    bonus = envFloat("RSZ_LOAD_DELAY_XP5_BONUS", 1.8e-11f);
  }
  if (bonus > 0.0f
      && (name.find("_R") != std::string::npos
          || name.find("_RVT") != std::string::npos)) {
    bonus += envFloat("RSZ_LOAD_DELAY_R_WEAK_BONUS", 6.0e-12f);
  }
  return bonus;
}
...
const char *RepairTargetCollector::getEnumString(ViolatorSortType sort_type) {
  switch (sort_type) {
...
void RepairTargetCollector::printViolators(int numPrint = 0) const {
  ...
  debugPrint(logger_, RSZ, "violator_collector", 23,
  ...
  for (const sta::Pin *pin : violating_pins_) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 2,
               "{} ({}) slack={} tns={} level={} delay={} "
               "(load_delay={} + "
               "intrinsic_delay={})",
               vertex->name(network_), cell->name(),
               delayAsString(slack, 3, sta_), delayAsString(tns, 3, sta_),
               pin_data_.at(pin).level, delayAsString(total_delay, 3, sta_),
               delayAsString(load_delay, 3, sta_),
               delayAsString(intrinsic_delay, 3, sta_));
  }
}

int RepairTargetCollector::getTotalViolations() const {
  return violating_endpoints_.size();
}

void RepairTargetCollector::printHistogram(int numBins) const {
  if (violating_pins_.empty()) {
    debugPrint(logger_, RSZ, "violator_collector", 1,
               "No violating pins found.");
    return;
  }
  ...
  for (const sta::Pin *pin : violating_pins_) {
    float slack = sta_->slack(pin, sta::RiseFall::rise()->asRiseFallBoth(),
                              sta_->scenes(), max_);
  ...
  debugPrint(logger_, RSZ, "violator_collector", 1,
             ...
             violating_pins_.size(), delayAsString(min_slack, 3, sta_),
  ...
  for (const sta::Pin *pin : violating_pins_) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 4,
               "Pin: {} Slack: {} Bucket: {}", network_->pathName(pin),
               delayAsString(slack, 3, sta_), slack_bucket);
  ...
  for (int i = min_bin_index; i <= max_bin_index; ++i) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 1,
               "Slack: < {:7}: ({:4}) {}", delayAsString(bin_slack, 3, sta_),
               count, bar);
  }
}
...
void RepairTargetCollector::collectBySlack() {
  violating_pins_.clear();
    ...
    while (pin_iter->hasNext()) {
      ...
      if (sta::fuzzyLess(slack, slack_margin_)) {
        ...
        debugPrint(logger_, RSZ, "violator_collector", 4,
                   ...
                   network_->pathName(pin), cell->name(),
                   delayAsString(slack, 3, sta_), vertex->level());
        violating_pins_.push_back(pin);
      }
    }
  }

  debugPrint(logger_, RSZ, "violator_collector", 2, "Found {} violating pins.",
             violating_pins_.size());
}

std::pair<sta::Delay, sta::Delay>
RepairTargetCollector::getEffortDelays(const sta::Pin *pin) {
  ...
  while (edge_iter.hasNext()) {
    ...
    for (const sta::RiseFall *rf : sta::RiseFall::range()) {
      ...
      debugPrint(logger_, RSZ, "violator_collector", 5,
                 ...
                 "Delay: {} Slack: {}",
                 network_->pathName(from_vertex->pin()), from_rf->name(),
                 network_->pathName(pin), rf->name(),
                 delayAsString(load_delay, 3, sta_),
                 delayAsString(intrinsic_delay, 3, sta_),
...
void RepairTargetCollector::updatePinData(const sta::Pin *pin, pinData &pd) {
  ...
  pd.slack = sta_->slack(pin, sta::RiseFall::rise()->asRiseFallBoth(),
                         sta_->scenes(), max_);
...
void RepairTargetCollector::sortByLoadDelay(float load_delay_threshold) {
  ...
  for (auto pin : violating_pins_) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 4,
               ...
               vertex->name(network_), delayAsString(worst_load_delay, 3, sta_),
...
sta::Slack
RepairTargetCollector::getPathSlackByIndex(const sta::Pin *endpoint_pin,
                                           int path_index) {
  // Create ExceptionTo for this endpoint
  sta::PinSet *to_pins = new sta::PinSet(network_);
  to_pins->insert(endpoint_pin);
  sta::ExceptionTo *to = sdc_->makeExceptionTo(to_pins, nullptr, nullptr,
...
RepairTargetCollector::collectViolators(int numPathsPerEndpoint, int numPins,
  ...
  if (current_endpoint_) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 2,
               ...
               network_->pathName(endpoint_pin), numPathsPerEndpoint);
    ...
    debugPrint(logger_, RSZ, "violator_collector", 3,
               "Collected {} unique pins from current endpoint's {} path(s)",
               collected_pins.size(), numPathsPerEndpoint);
    ...
    while ((numPins == -1 ||
      ...
      sta::Slack best_endpoint_slack = sta::INF;
      int best_path_num = 0;
      ...
      for (const auto &[endpoint_pin, endpoint_slack] : violating_endpoints_) {
        ...
        if (times_considered >= max_passes_per_endpoint_) {
          debugPrint(logger_, RSZ, "violator_collector", 4,
                     ...
                     network_->pathName(endpoint_pin), times_considered,
        ...
        if (paths_used < numPathsPerEndpoint) {
          ...
          if (betterGlobalSetupEndpoint(path_slack, endpoint_slack, best_slack,
                                        best_endpoint_slack)) {
            best_slack = path_slack;
            best_endpoint_slack = endpoint_slack;
      ...
      debugPrint(
          logger_, RSZ, "violator_collector", 3,
          "Collected {} pins ({} new) from endpoint {} path {} (slack={})",
          new_pins.size(), collected_pins.size() - before_size,
          network_->pathName(best_endpoint), best_path_num + 1,
  ...
  violating_pins_.clear();
  violating_pins_.insert(violating_pins_.end(), collected_pins.begin(),
                         collected_pins.end());

  debugPrint(logger_, RSZ, "violator_collector", 2,
             "Collected {} unique violating pins", violating_pins_.size());
  ...
  debugPrint(logger_, RSZ, "violator_collector", 4,
             "Endpoint {} pass count: {}/{}",
             network_->pathName(current_endpoint_->pin()), current_pass_count_,
             max_passes_per_endpoint_);

  return violating_pins_;
}

void RepairTargetCollector::sortPins(int numPins, ViolatorSortType sort_type) {
  for (auto pin : violating_pins_) {
    updatePinData(pin, pin_data_[pin]);
  }
  debugPrint(logger_, RSZ, "violator_collector", 1,
             "Sorting {} violating pins by {}.", violating_pins_.size(),
...
void RepairTargetCollector::sortByWns() {
  std::ranges::sort(
...
sta::Delay RepairTargetCollector::getLocalPinTns(const sta::Pin *pin) const {
  sta::Delay tns = 0;
  ...
  while (edge_iter.hasNext()) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 4,
               " pin {} fanout {} slack: {} tns: {}", network_->pathName(pin),
...
map<const sta::Pin *, sta::Delay> RepairTargetCollector::getLocalTns() const {
...
void RepairTargetCollector::sortByLocalTns() {
  std::ranges::sort(
      violating_pins_, [this](const sta::Pin *a, const sta::Pin *b) {
        float tns1 = pin_data_[a].tns;
        float tns2 = pin_data_[b].tns;
...
void RepairTargetCollector::sortByHeuristic(float load_delay_threshold) {
  std::ranges::sort(
      violating_pins_, [this](const sta::Pin *a, const sta::Pin *b) {
        ...
        score_a += weakDriverHeuristicBonus(network_, a);
        score_b += weakDriverHeuristicBonus(network_, b);
...
void RepairTargetCollector::collectViolatingEndpoints() {
  ...
  std::ranges::stable_sort(violating_endpoints_, slackLess);

  debugPrint(logger_, RSZ, "violator_collector", 1,
             "Violating endpoints {}/{} {}%", violating_endpoints_.size(),
             sta_->endpoints().size(),
             int(violating_endpoints_.size() /
                 double(sta_->endpoints().size()) * 100));
}
...
void RepairTargetCollector::collectViolatingStartpoints() {
  ...
  std::ranges::stable_sort(violating_startpoints_, slackLess);

  debugPrint(
      logger_, RSZ, "violator_collector", 1, "Violating startpoints {}/{} {}%",
      violating_startpoints_.size(), all_startpoints,
      all_startpoints > 0
          ? int(violating_startpoints_.size() / double(all_startpoints) * 100)
          : 0);
}

sta::Path *
RepairTargetCollector::findWorstSlackPath(sta::Vertex *endpoint) const {
  return sta_->vertexWorstSlackPath(endpoint, max_);
}

std::vector<Target> RepairTargetCollector::collectPathDriverTargets(
    sta::Path *path, const sta::Slack path_slack) const {
  ...
  for (int index = expanded.startIndex(); index < expanded.size(); ++index) {
    ...
    target.fanout = wireFanout(vertex, graph_);
...
std::vector<Target> RepairTargetCollector::collectCritPathDriverPinTargets(
    const std::function<bool(sta::Pin *)> &is_valid_target) const {
  std::vector<VertexSlack> violated_endpoints;
  violated_endpoints.reserve(violating_endpoints_.size());
  for (const PinSlack &endpoint_data : violating_endpoints_) {
    const sta::Pin *endpoint_pin = endpoint_data.first;
    const sta::Slack endpoint_slack = endpoint_data.second;
    sta::Vertex *endpoint_vertex = graph_->pinLoadVertex(endpoint_pin);
    if (endpoint_vertex != nullptr) {
      violated_endpoints.emplace_back(endpoint_vertex, endpoint_slack);
    }
  }

  // Preserve equal-slack endpoint order from collectViolatingEndpoints().
  std::ranges::stable_sort(violated_endpoints, pathEndpointLess);

  std::vector<Target> targets;
  std::unordered_set<sta::Pin *> visited_pins;
  for (const VertexSlack &endpoint_data : violated_endpoints) {
    sta::Vertex *endpoint = endpoint_data.first;
    const sta::Slack endpoint_slack = endpoint_data.second;
    ...
    for (Target &target : collectPathDriverTargets(path, endpoint_slack)) {
      sta::Pin *pin = target.driver_pin;
...
sta::Slack
RepairTargetCollector::getStartpointWns(const sta::Pin *startpoint_pin) const {
...
sta::Slack
RepairTargetCollector::getStartpointTns(const sta::Pin *startpoint_pin) const {
...
sta::Slack
RepairTargetCollector::getEndpointWns(const sta::Pin *endpoint_pin) const {
...
sta::Slack
RepairTargetCollector::getEndpointTns(const sta::Pin *endpoint_pin) const {
...
void RepairTargetCollector::collectByPaths(int endPointIndex, int numEndpoints,
  ...
  for (int i = endPointIndex; i < violating_endpoints_.size(); i++) {
    ...
    if (sta::fuzzyLess(end_original_slack.second, slack_margin_)) {
      ...
      debugPrint(
          logger_, RSZ, "violator_collector", 2,
          "Collected {} pins ({} unique) for endpoint {} total collected {}",
          end_pins.size(), viol_pins.size() - old_size,
          network_->pathName(endpoint_pin), viol_pins.size());
  ...
  violating_pins_.clear();
  violating_pins_.insert(violating_pins_.end(), viol_pins.begin(),
                         viol_pins.end());
}

set<const sta::Pin *>
RepairTargetCollector::collectPinsByPathEndpoint(const sta::Pin *endpoint_pin,
...

{
  ...
  for (const sta::PathEnd *path_end : path_ends) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 4,
               "Critical path {} (Slack: {}ps) Pins: {}", path_num++,
               delayAsString(path_slack, 3, sta_), expanded.size());
    ...
    for (size_t i = 0; i < expanded.size(); i++) {
      ...
      if (network_->isTopLevelPort(pin)) {
        debugPrint(logger_, RSZ, "violator_collector", 5, "  - Port: {}",
...
vector<const sta::Pin *> RepairTargetCollector::collectViolatorsFromEndpoints(
    const vector<sta::Vertex *> &endpoints, int numPathsPerEndpoint,
    int numPins, ViolatorSortType sort_type) {
  violating_pins_.clear();

  debugPrint(logger_, RSZ, "violator_collector", 2,
             "Collecting violators from {} specific endpoints: paths={}, "
             "pins={}, sort={}",
             endpoints.size(), numPathsPerEndpoint, numPins,
             getEnumString(sort_type));
  ...
  for (sta::Vertex *endpoint : endpoints) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 3,
               "Processing endpoint: {} (slack={})", endpoint->name(network_),
  ...
  debugPrint(logger_, RSZ, "violator_collector", 2,
             ...
             violating_pins_.size(), endpoints.size());
...
vector<const sta::Pin *>
RepairTargetCollector::collectViolatorsByPin(int numPins,
                                             ViolatorSortType sort_type) {
  debugPrint(logger_, RSZ, "violator_collector", 2,
             "Collecting violators by pin slack: pins={}, sort={}", numPins,
             getEnumString(sort_type));
  ...
  debugPrint(logger_, RSZ, "violator_collector", 2,
             "Found {} total violating pins", violating_pins_.size());

  // Sort and limit the pins
  sortPins(numPins, sort_type);

  debugPrint(logger_, RSZ, "violator_collector", 2,
             "After sorting and limiting: {} pins", violating_pins_.size());
...
vector<const sta::Pin *>
RepairTargetCollector::collectViolatorsBySlackMargin(float slack_margin) {
  // Use efficient fanin cone traversal instead of whole-design scan
  return collectViolatorsByFaninTraversal(slack_margin);
}

vector<const sta::Pin *>
RepairTargetCollector::collectViolatorsByFaninTraversal(
    float slack_margin, ViolatorSortType sort_type) {
  ...
  debugPrint(logger_, RSZ, "violator_collector", 2,
             ...
             delayAsString(slack_threshold, 3, sta_), slack_margin);
  ...
  debugPrint(logger_, RSZ, "violator_collector", 2,
             ...
             critical_endpoints.size(), all_endpoints.size());
  ...
  while (!to_visit.empty()) {
    ...
    while (edge_iter.hasNext()) {
      ...
      if (sta_->isClock(from_pin, sta_->cmdMode())) {
        debugPrint(logger_, RSZ, "violator_collector", 4,
                   "Stopping at clock pin: {}", network_->pathName(from_pin));
        continue;
      }
      ...
      if (network_->direction(from_pin)->isOutput()) {
        ...
        sta::Slack pin_slack =
            sta_->slack(from_pin, sta::RiseFall::rise()->asRiseFallBoth(),
                        sta_->scenes(), max_);
  ...
  debugPrint(logger_, RSZ, "violator_collector", 2,
             ...
             vertices_processed, collected_pins_set.size());
  ...
  debugPrint(logger_, RSZ, "violator_collector", 2,
             ...
             violating_pins_.size(), pins_before_filter);
...
vector<const sta::Pin *>
RepairTargetCollector::collectViolatorsByFanoutTraversal(
    sta::Vertex *startpoint, ViolatorSortType sort_type,
  ...
  if (startpoint_pin) {
    sta::Slack pin_slack =
        sta_->slack(startpoint_pin, sta::RiseFall::rise()->asRiseFallBoth(),
                    sta_->scenes(), max_);
    if (pin_slack < slack_threshold) {
        ...
        if (inst) {
          ...
          while (pin_iter->hasNext()) {
            ...
            if (network_->direction(inst_pin)->isOutput()) {
              ...
              if (out_vertex) {
                // Check slack criterion
                sta::Slack pin_slack = sta_->slack(
                    inst_pin, sta::RiseFall::rise()->asRiseFallBoth(),
                    sta_->scenes(), max_);
                if (pin_slack < slack_threshold) {
                  // Add to collected pins
...
vector<const sta::Pin *>
RepairTargetCollector::collectViolatorsByFaninTraversalForEndpoint(
    sta::Vertex *endpoint, float slack_margin, ViolatorSortType sort_type) {
  // Get worst endpoint slack as reference
  sta::Slack worst_slack;
  sta::Vertex *worst_vertex;
  sta_->worstSlack(max_, worst_slack, worst_vertex);
  ...
  debugPrint(
      ...
      network_->pathName(endpoint_pin), delayAsString(endpoint_slack, 3, sta_),
      delayAsString(worst_slack, 3, sta_),
      delayAsString(slack_threshold, 3, sta_), slack_margin);
  ...
  debugPrint(logger_, RSZ, "violator_collector", 3,
             ...
             network_->pathName(endpoint_pin), vertices_processed,
...
sta::Slack RepairTargetCollector::computeAdaptiveThreshold(
    ...
    sta::Slack endpoint_slack, int &pin_count) {
  ...
  if (cone_size > 0) {
    for (float margin_pct : margin_percentages) {
      ...
      debugPrint(logger_, RSZ, "violator_collector", 1,
                 ...
                 "threshold={}, pin_count={}",
                 margin_pct * 100.0, delayAsString(endpoint_slack, 3, sta_),
                 delayAsString(margin, 3, sta_),
                 delayAsString(threshold, 3, sta_), count);
      ...
      if (count >= min_target_pins && count <= max_target_pins) {
        ...
        logger_->info(
            ...
            margin_pct * 100.0, delayAsString(chosen_threshold, 3, sta_),
            pin_count, cone_size, 100.0 * pin_count / cone_size);
...
void RepairTargetCollector::collectPinsWithThreshold(
    const std::vector<std::pair<const sta::Pin *, sta::Slack>> &pins_with_slack,
    sta::Slack threshold) {
  violating_pins_.clear();
  pin_data_.clear();
...
vector<const sta::Pin *> RepairTargetCollector::collectViolatorsByConeTraversal(
    sta::Vertex *endpoint, ViolatorSortType sort_type,
  ...
  if (explicit_threshold.has_value()) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 2,
               ...
               violating_pins_.size(), network_->pathName(endpoint_pin));
  }
...
void RepairTargetCollector::setMaxPassesPerEndpoint(int max_passes) {
  max_passes_per_endpoint_ = max_passes;
}

bool RepairTargetCollector::shouldSkipEndpoint() const {
  return current_pass_count_ >= max_passes_per_endpoint_;
}

int RepairTargetCollector::getEndpointPassCount() const {
  return current_pass_count_;
}

void RepairTargetCollector::resetEndpointPasses() {
  current_pass_count_ = 0;
  endpoint_times_considered_.clear();
  debugPrint(logger_, RSZ, "violator_collector", 2,
...
bool RepairTargetCollector::hasMoreEndpoints() const {
  if (iteration_began_) {
...
void RepairTargetCollector::setToEndpoint(int index) {
...
void RepairTargetCollector::advanceToNextEndpoint() {
  if (hasMoreEndpoints()) {
    ...
    debugPrint(logger_, RSZ, "violator_collector", 2,
               "Advancing to next endpoint {}/{} ({})", current_endpoint_index_,
               violating_endpoints_.size(),
               network_->pathName(
...
sta::Slack RepairTargetCollector::getCurrentEndpointSlack() const {
  if (current_endpoint_) {
...
void RepairTargetCollector::useWorstEndpoint(sta::Vertex *end) {
  current_endpoint_ = end;
}

int RepairTargetCollector::getCurrentPass() const {
  return current_pass_count_;
}

void RepairTargetCollector::markEndpointVisitedInWns(const sta::Pin *endpoint) {
  wns_visited_endpoints_.insert(endpoint);
}

bool RepairTargetCollector::wasEndpointVisitedInWns(
    const sta::Pin *endpoint) const {
  return wns_visited_endpoints_.contains(endpoint);
}

void RepairTargetCollector::clearWnsVisitedEndpoints() {
  wns_visited_endpoints_.clear();
}

void RepairTargetCollector::markPinConsidered(const sta::Pin *pin) {
  considered_pins_.insert(pin);
}

bool RepairTargetCollector::wasPinConsidered(const sta::Pin *pin) const {
  return considered_pins_.contains(pin);
}

void RepairTargetCollector::clearConsideredPins() { considered_pins_.clear(); }

const std::set<const sta::Pin *> &
RepairTargetCollector::getConsideredPins() const {
  return considered_pins_;
}

std::vector<const sta::Pin *>
RepairTargetCollector::getCriticalPinsNeverConsidered() {
  ...
  std::ranges::sort(never_considered, [this](const sta::Pin *a,
                                             const sta::Pin *b) {
    sta::Slack slack_a = sta_->slack(a, sta::RiseFall::rise()->asRiseFallBoth(),
                                     sta_->scenes(), max_);
    sta::Slack slack_b = sta_->slack(b, sta::RiseFall::rise()->asRiseFallBoth(),
                                     sta_->scenes(), max_);
...
sta::Slack RepairTargetCollector::getOverallStartpointWns() const {
...
sta::Slack RepairTargetCollector::getOverallStartpointTns(bool use_cone) const {
...
sta::Slack RepairTargetCollector::getOverallEndpointWns() const {
  sta::Slack worst_slack = std::numeric_limits<float>::max();
...
sta::Slack RepairTargetCollector::getTns(bool use_startpoints,
                                         bool use_cone) const {
  if (use_startpoints) {
...
// endpoints
const sta::Pin *RepairTargetCollector::getWorstPin(bool use_startpoints) const {
  if (use_startpoints) {
...
void RepairTargetCollector::collectViolatingPoints(bool use_startpoints) {
  if (use_startpoints) {
    collectViolatingStartpoints();
  } else {
    collectViolatingEndpoints();
...
int RepairTargetCollector::getMaxPointCount(bool use_startpoints) const {
  if (use_startpoints) {
...
void RepairTargetCollector::setToPoint(int index, bool use_startpoints) {
  if (use_startpoints) {
...
sta::Vertex *
RepairTargetCollector::getCurrentPoint(bool use_startpoints) const {
  if (use_startpoints) {
...
const sta::Pin *
RepairTargetCollector::getCurrentPointPin(bool use_startpoints) const {
...
// consistency between the RepairTargetCollector's iteration state and the
// collection being performed.
vector<const sta::Pin *>
RepairTargetCollector::collectViolatorsByDirectionalTraversal(
    bool use_startpoints, int point_index, sta::Slack slack_threshold,
    ViolatorSortType sort_type) {
  if (use_startpoints) {
    // For startpoints, use fanout traversal
    setToStartpoint(point_index);
    sta::Vertex *startpoint = getCurrentStartpoint();
    return collectViolatorsByFanoutTraversal(startpoint, sort_type,
                                             slack_threshold);
  }
