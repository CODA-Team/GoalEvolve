...
constexpr float kReclaimSlackFloor = 0.022f;
constexpr float kReclaimCriticalSlackFloor = -0.018f;
constexpr float kReclaimDelayTax = 1.70f;
...
bool isFastLeakageCell(const sta::LibertyCell *cell) {
  const std::string name = cell != nullptr ? cell->name() : "";
  return name.find("_SL") != std::string::npos ||
         name.find("SLVT") != std::string::npos ||
...
bool resolveDriverContext(SizeDownContext &ctx) {
  ...
  if (ctx.target.fanout >= kSizeDownMaxFanout) {
    debugPrint(ctx.resizer.logger(), RSZ, "size_down_move", 2,
               "REJECT SizeDownMove {}: Fanout {} >= {} max fanout",
               ctx.resizer.network()->pathName(ctx.drvr_pin), ctx.target.fanout,
               kSizeDownMaxFanout);
    return false;
  }
...
sortedFanoutSlacks(const SizeDownContext &ctx) {
  ...
  while (edge_iter.hasNext()) {
    ...
    const sta::Slack fanout_slack =
        ctx.resizer.sta()->slack(fanout_vertex, ctx.resizer.maxAnalysisMode());
  ...
  std::ranges::sort(fanout_slacks, [&ctx](const auto &lhs, const auto &rhs) {
    return lhs.second > rhs.second || (lhs.second == rhs.second &&
                                       ctx.resizer.network()->pathNameLess(
...
bool resolveLoadContext(const SizeDownContext &ctx, sta::Vertex *load_vertex,
  ...
  if (load_ctx.load_inst == nullptr ||
      ctx.resizer.dontTouch(load_ctx.load_inst) ||
      !ctx.resizer.isLogicStdCell(load_ctx.load_inst) ||
      ctx.resizer.drivesSequentialClockPin(load_ctx.load_inst)) {
    return false;
  }
  if (load_slack < kReclaimCriticalSlackFloor) {
    debugPrint(
        ctx.resizer.logger(), RSZ, "size_down_move", 3,
        " reject reclaim candidate {}: endpoint slack {} below forced "
        "reclaim damage floor {}",
        ctx.resizer.network()->pathName(load_ctx.load_pin),
        delayAsString(load_slack, 3, ctx.resizer.staState()),
        delayAsString(kReclaimCriticalSlackFloor, 3, ctx.resizer.staState()));
    return false;
  }
...
sta::Slack getWorstOutputSlack(const SizeDownContext &ctx,
                               sta::Instance *inst) {
  sta::Slack worst_slack = sta::INF;
  auto pin_iter = std::unique_ptr<sta::InstancePinIterator>(
      ctx.resizer.network()->pinIterator(inst));
...
bool checkMaxCapViolation(const SizeDownContext &ctx,
  ...
  output_port->capacitanceLimit(ctx.resizer.maxAnalysisMode(), max_cap,
                                cap_limit_exists);
  if (cap_limit_exists && max_cap > 0.0f && output_cap > max_cap) {
    debugPrint(ctx.resizer.logger(), RSZ, "opt_moves", 2,
               "  skip based on max cap {} gate={} cap={} max_cap={}",
               ctx.resizer.network()->pathName(output_pin),
               output_port->libertyCell()->name(), output_cap, max_cap);
    return true;
  }
  return false;
}
...
float computeElmoreSlewFactor(const SizeDownContext &ctx,
  ...
  const sta::Slew slew = ctx.resizer.sta()->slew(
      ctx.resizer.graph()->pinLoadVertex(output_pin),
      sta::RiseFallBoth::riseFall(), ctx.resizer.sta()->scenes(),
...
bool checkMaxSlewViolation(const SizeDownContext &ctx,
  ...
  ctx.resizer.sta()->findSlewLimit(output_port, ctx.scene,
                                   ctx.resizer.maxAnalysisMode(), max_slew,
                                   slew_limit_exists);
  if (slew_limit_exists && new_slew > max_slew) {
    debugPrint(ctx.resizer.logger(), RSZ, "opt_moves", 2,
               "  skip based on max slew {} gate={} slew={} max_slew={}",
               ctx.resizer.network()->pathName(output_pin),
               output_port->libertyCell()->name(), new_slew, max_slew);
    return true;
  }
  return false;
}
...
void logSwappableCells(const SizeDownContext &ctx,
  ...
  if (ctx.resizer.logger()->debugCheck(RSZ, "size_down_move", 3)) {
    ...
    debugPrint(ctx.resizer.logger(), RSZ, "size_down_move", 3,
...
SizeDownOutputProfile buildOutputProfile(const SizeDownContext &ctx,
  ...
  for (const sta::Pin *output_pin : profile.output_pins) {
    ...
    const float load_delay = ctx.resizer.gateDelay(output_port, output_load_cap,
                                                   ctx.scene, ctx.min_max);
    profile.output_delays.push_back(load_delay);

    debugPrint(
        ctx.resizer.logger(), RSZ, "size_down_move", 4,
        " current {}->{} gate={} delay={} cap={} slew={} slack={}",
        ctx.resizer.network()->pathName(load_ctx.load_pin),
        ctx.resizer.network()->pathName(output_pin), load_ctx.load_cell->name(),
        delayAsString(load_delay, 3, ctx.resizer.staState()), output_load_cap,
...
sta::Slack computeDelayBudget(const SizeDownContext &ctx,
  ...
  debugPrint(ctx.resizer.logger(), RSZ, "size_down_move", 4,
...
bool violatesOutputLimits(const SizeDownContext &ctx,
  ...
  for (size_t i = 0; i < profile.output_pins.size(); ++i) {
    ...
    if (checkMaxCapViolation(ctx, profile.output_pins[i], output_port,
                             profile.output_caps[i]) ||
        checkMaxSlewViolation(ctx, profile.output_pins[i], output_port,
...
bool fitsDelayBudget(const SizeDownContext &ctx,
  ...
  const sta::Slack reclaim_budget =
      load_ctx.delay_budget > kReclaimSlackFloor
          ? sta::Slack((load_ctx.delay_budget - kReclaimSlackFloor) /
                       kReclaimDelayTax)
          : load_ctx.delay_budget;
  const bool violates_budget =
      (load_ctx.delay_budget > 0 && worst_delay_change > reclaim_budget) ||
  ...
  if (violates_budget) {
    debugPrint(
        ...
        delayAsString(first_new_load_delay, 3, ctx.resizer.staState()),
        delayAsString(profile.output_delays[0], 3, ctx.resizer.staState()),
        delayAsString(reclaim_budget, 3, ctx.resizer.staState()),
        delayAsString(worst_delay_change, 3, ctx.resizer.staState()),
        delayAsString(reclaim_budget, 3, ctx.resizer.staState()));
  }
...
float reclaimScore(const SizeDownContext &ctx,
                   const SizeDownLoadContext &load_ctx,
                   sta::LibertyCell *swappable) {
  const float cap_drop = candidateInputCap(load_ctx, load_ctx.load_cell) -
                         candidateInputCap(load_ctx, swappable);
  const float area_drop = load_ctx.load_cell->area() - swappable->area();
  const float leakage_drop =
      ctx.resizer.cellLeakage(load_ctx.load_cell).value_or(0.0f) -
      ctx.resizer.cellLeakage(swappable).value_or(0.0f);
  if (cap_drop <= 0.0f && area_drop <= 0.0f && leakage_drop <= 0.0f) {
    return -sta::INF;
  }
  const float leakage_bonus =
      isFastLeakageCell(load_ctx.load_cell) ? 2.0f : 0.0f;
  const float slack_reserve =
      std::max(0.0f, load_ctx.load_slack - kReclaimSlackFloor);
  return cap_drop * 10.0f + area_drop * 0.25f + leakage_drop * 1.0e6f +
         leakage_bonus + slack_reserve * 4.0f;
}

sta::LibertyCell *
selectReplacementCell(const SizeDownContext &ctx,
                      const SizeDownLoadContext &load_ctx,
                      const sta::LibertyCellSeq &swappable_cells,
                      const SizeDownOutputProfile &profile) {
  ...
  for (sta::LibertyCell *swappable : swappable_cells) {
    ...
    debugPrint(ctx.resizer.logger(), RSZ, "size_down_move", 4,
               " considering swap {} {} -> {}",
               ctx.resizer.network()->pathName(load_ctx.load_pin),
               load_ctx.load_cell->name(), swappable->name());

    if (isWorseCapOrArea(load_ctx, best_cell, swappable)) {
      debugPrint(ctx.resizer.logger(), RSZ, "size_down_move", 4,
                 "  skip based on cap/area {} gate={} cap={}>{} area={}>{}",
                 ctx.resizer.network()->pathName(load_ctx.load_pin),
                 swappable->name(), candidateInputCap(load_ctx, swappable),
                 candidateInputCap(load_ctx, best_cell), swappable->area(),
                 best_cell->area());
      continue;
    }
    ...
    const float score = reclaimScore(ctx, load_ctx, swappable);
    if (score <= best_score) {
      continue;
    }
...
std::unique_ptr<MoveCandidate> buildCandidate(const SizeDownContext &ctx,
  ...
  if (replacement == nullptr) {
    debugPrint(ctx.resizer.logger(), RSZ, "size_down_move", 3,
               ...
               ctx.resizer.network()->pathName(load_ctx.load_pin),
               load_ctx.load_cell->name(), "none",
               delayAsString(load_ctx.load_slack, 3, ctx.resizer.staState()));
    return nullptr;
  }

  return std::make_unique<SizeDownCandidate>(
      ctx.resizer, ctx.target, ctx.drvr_pin, load_ctx.load_inst,
      load_ctx.load_pin, load_ctx.load_cell, replacement, load_ctx.load_slack);
}

std::vector<std::unique_ptr<MoveCandidate>>
buildCandidates(const SizeDownContext &ctx) {
  debugPrint(ctx.resizer.logger(), RSZ, "size_down_move", 2,
...
std::vector<std::unique_ptr<MoveCandidate>>
SizeDownGenerator::generate(const Target &target) {
  SizeDownContext ctx{
