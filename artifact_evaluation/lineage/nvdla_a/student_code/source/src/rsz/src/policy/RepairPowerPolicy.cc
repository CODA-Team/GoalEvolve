...
void RepairPowerPolicy::iterateEarlyForcedReclaim() {
  ...
  const bool power_clone_enabled = config_.repair_power_command &&
                                   config_.power_phase ==
                                       "early_forced_reclaim";
  const auto power_clone_score = [this](const Candidate &candidate) {
    const double leakage_ratio =
        candidate.leakage_gain / positiveOrDefault(candidate.target.leakage, 1.0);
    const bool strict_vt =
        candidate.replacement != nullptr &&
        isStrictPowerDirectionVtSwap(candidate.target.cell,
                                     candidate.replacement);
    const double vt_bonus = strict_vt ? 0.050 : 0.0;
    const double cap_bonus =
        std::max(0.0, candidate.input_cap_gain) * cfg_.input_cap_weight * 0.05;
    const double fanout_penalty =
        std::max(0, candidate.target.fanout - 8) * 0.001;
    return leakage_ratio + vt_bonus + cap_bonus - fanout_penalty;
  };
  bool changed = false;
  ...
  const auto moveLimitReached = [&]() {
    ...
    return (!cfg_.bounded_early_tail && !power_clone_enabled) ||
  ...
  while (!moveLimitReached() && !trialLimitReached()) {
    ...
    if (committed_ >= retained_move_cap && power_clone_enabled) {
      candidates.erase(
          std::remove_if(candidates.begin(), candidates.end(),
                         [](const Candidate &candidate) {
                           return candidate.leakage_gain <= 0.0;
                         }),
          candidates.end());
      for (Candidate &candidate : candidates) {
        candidate.score = power_clone_score(candidate);
      }
    }
    ...
    for (size_t i = 0; i < candidates.size();) {
      ...
      const bool clone_candidate = tail_candidate && power_clone_enabled;
      if (tail_candidate) {
        ++tail_examined;
      ...
      if (clone_candidate) {
        logger_->report("METRIC|repair_power_clone_examined|1");
      }
      if (tryCommitCandidate(candidate, current)) {
        ...
        if (clone_candidate) {
          logger_->report("METRIC|repair_power_clone_retained|1");
        }
        touched_.insert(candidate.target.inst);
      } else {
        ++rejected_;
  ...
  if (power_clone_enabled && committed_ < retained_move_cap) {
    logger_->report(
        "REPAIR_POWER|clone|status=nonactivated|reason=retained_cap_not_reached");
  }
...
bool RepairPowerPolicy::tryCommitCandidate(const Candidate &candidate,
  ...
  if (config_.repair_power_command &&
    ...
    sta::LibertyCell *applied_cell =
        candidate.kind == MoveKind::kRemoveBuffer
            ? nullptr
            : network_->libertyCell(candidate.target.inst);
    const double measured_leakage_gain =
        candidate.kind == MoveKind::kRemoveBuffer
            ? candidate.target.leakage
            : (applied_cell != nullptr
                   ? candidate.target.leakage - cellLeakage(applied_cell)
                   : 0.0);
    if (measured_leakage_gain <= 0.0) {
      resizer_.journalRestore();
      recordReject("measured_leakage");
      return false;
    }
  }
