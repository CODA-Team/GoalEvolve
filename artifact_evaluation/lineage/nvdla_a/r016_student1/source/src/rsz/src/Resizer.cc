...
bool Resizer::drivesSequentialClockPin(const sta::Instance* inst) const
{
  if (inst == nullptr || graph_ == nullptr) {
    return false;
  }
  ...
  while (pin_iter->hasNext()) {
    ...
    while (edge_iter.hasNext()) {
      ...
      if (fanout_vertex != nullptr && fanout_vertex->isRegClk()) {
        return true;
      }
    }
  }
                              ...
                              const bool honor_dont_touch_fixed)
{
  ...
  if (drivesSequentialClockPin(buffer)) {
    return false;
  }
...
bool Resizer::replaceCell(sta::Instance* inst,
                          ...
                          const bool journal)
{
  if (drivesSequentialClockPin(inst)) {
    return false;
  }
...
////////////////////////////////////////////////////////////////
bool Resizer::repairPower(const char* phase,
                          float proportion,
                          int max_moves,
                          double max_tns_expand_ratio,
                          double max_wns_drop,
                          bool match_cell_footprint,
                          bool verbose)
{
  ...
  rsz::OptimizerRunConfig config;
  config.setup_slack_margin = 0.0;
  config.repair_tns_end_percent = 1.0;
  config.max_passes = 1;
  config.max_iterations = 1;
  config.max_repairs_per_pass = 1;
  config.match_cell_footprint = match_cell_footprint;
  config.verbose = verbose;
  config.phases = "REPAIR_POWER";
  config.repair_power_command = true;
  config.power_phase = phase != nullptr ? phase : "";
  config.power_proportion = std::clamp(proportion, 0.0f, 1.0f);
  config.power_max_moves = max_moves;
  config.power_max_tns_expand_ratio = max_tns_expand_ratio;
  config.power_max_wns_drop = max_wns_drop;
  ...
  if (!config.power_phase.empty()
      && config.power_phase != "early_forced_reclaim") {
    return true;
  }

  rsz::OptimizerRunConfig late_config = config;
  late_config.power_phase = "late_leakage_recovery_second_pass";
  late_config.power_max_moves = 0;
  late_config.power_max_tns_expand_ratio = 0.0;
  late_config.power_max_wns_drop = 0.0;

  rsz::Optimizer late_optimizer(this);
  late_optimizer.configure(late_config);
  return late_optimizer.run();
}
...
void Resizer::journalEndNoTimingUpdate()
{
  debugPrint(logger_, RSZ, "journal", 1, "journal end no timing update");
  odb::dbDatabase::commitEco(block_);
}
...
sta::Instance* Resizer::insertBufferBeforeLoad(
    ...
    const odb::dbNameUniquifyType& uniquify)
{
  if (load_pin != nullptr && sta_->isClock(load_pin, sta_->cmdMode())) {
    return nullptr;
  }
  sta::Net* net = load_pin != nullptr ? network_->net(load_pin) : nullptr;
  if (net != nullptr && sta_->isClock(net, sta_->cmdMode())) {
    return nullptr;
  }
  sta::Vertex* vertex
      = load_pin != nullptr ? graph_->pinLoadVertex(load_pin) : nullptr;
  if (vertex != nullptr && vertex->isRegClk()) {
    return nullptr;
  }
    ...
    const odb::dbNameUniquifyType& uniquify)
{
  ...
  if (sta_net != nullptr && sta_->isClock(sta_net, sta_->cmdMode())) {
    return nullptr;
  }
