  ...
  while (committed_moves_ < power_config_.max_moves &&
    ...
    for (const TargetInfo &target : targets) {
      ...
      for (TrialCandidate &candidate : candidates) {
        ...
        ++trial_count_;
        const bool measured = measureCandidate(candidate, current);
        if (measured && power_config_.phase == "late_leakage_recovery") {
          logger_->report("METRIC|power_recovery_plus_late_examined|1");
        }
        if (measured &&
...
bool PowerRecoveryPlusPolicy::commitCandidate(const TrialCandidate &candidate) {
  ...
  if (power_config_.phase == "late_leakage_recovery") {
    logger_->report("METRIC|power_recovery_plus_late_retained|1");
  }
