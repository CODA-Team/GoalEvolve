...
static constexpr float kWnsTnsExpansionCapRatio = 0.018f;
static constexpr float kWnsGainFloor = 2.0e-12f;
...
void SetupWnsPolicy::iterate() {
  ...
  repairSetupWns(config_.setup_slack_margin, config_.max_passes,
...
void SetupWnsPolicy::repairSetupWns(const float setup_slack_margin,
  ...
  if (worst_endpoint == nullptr) {
    debugPrint(logger_, RSZ, "repair_setup", 1,
               "WNS{} Phase: No worst endpoint found", phase_marker);
    return;
  }

  debugPrint(logger_, RSZ, "repair_setup", 1,
             "WNS{} Phase: Focusing on worst slack path{}...", phase_marker,
  ...
  while (true) {
    ...
    if (endpoint_pass_counts[endpoint_pin] >= max_passes_per_endpoint) {
      debugPrint(logger_, RSZ, "repair_setup", 1,
                 ...
                 "exiting",
                 phase_marker, network_->pathName(endpoint_pin),
    ...
    if (sta::fuzzyGreaterEqual(worst_slack, setup_slack_margin)) {
      debugPrint(logger_, RSZ, "repair_setup", 1,
                 ...
                 phase_marker, delayAsString(worst_slack, kDelayDigits, sta_),
    ...
    if (terminateProgress(
            ...
            use_cone_collection ? "WNS_CONE" : "WNS_PATH", phase_marker)) {
      ...
      if (setup_context_.overall_no_progress_count >= max_no_progress) {
        debugPrint(logger_, RSZ, "repair_setup", 1,
                   ...
                   phase_marker, setup_context_.overall_no_progress_count);
        break;
      }
    }
    ...
    bool changed;
    {
      ...
      changed = repairPins(viol_pins, focus_path,
                           &rejected_pin_moves_current_endpoint_, &chosen_moves,
                           force_single_repair);
    }
    ...
    const float wns_gain =
        std::max(0.0f, static_cast<float>(new_wns - prev_worst_slack));
    ...
    const bool within_tns_guard =
        tns_improved || tns_expand_ratio <= kWnsTnsExpansionCapRatio;
    const bool useful_wns_gain = wns_gain >= kWnsGainFloor || endpoint_improved;
    const bool better =
        within_tns_guard &&
        (wns_improved || (wns_same && (useful_wns_gain || tns_improved)));
  ...
  if (logger_->debugCheck(RSZ, "repair_setup", 1)) {
    ...
    debugPrint(logger_, RSZ, "repair_setup", 1,
               "WNS{} Phase complete. WNS: {}, TNS: {}", phase_marker,
