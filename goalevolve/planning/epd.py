...
class EvolutionProgramDatabase:
    ...
    def _idea_row(
        ...
        *,
        ...
        epd_search_query: str = "",
        retrieved_historical_ideas: Sequence[str] = (),
        opened_epd_records: Sequence[str] = (),
        nearest_historical_idea: str = "",
        ...
        return {
            ...
            "draft_signature_id": draft_signature_id,
            "epd_search_query": epd_search_query,
            "retrieved_historical_ideas": list(retrieved_historical_ideas),
            "opened_epd_records": list(opened_epd_records),
            "nearest_historical_idea": nearest_historical_idea,
        ...
        for priority, raw in enumerate(evolution_ideas):
            ...
            ideas.append(
                self._idea_row(
                    ...
                    draft_signature_id=str(source.get("draft_signature_id") or ""),
                    epd_search_query=str(source.get("epd_search_query") or ""),
                    retrieved_historical_ideas=tuple(str(item) for item in list(source.get("retrieved_historical_ideas") or ()) if item),
                    opened_epd_records=tuple(str(item) for item in list(source.get("opened_epd_records") or ()) if item),
                    nearest_historical_idea=str(source.get("nearest_historical_idea") or ""),
                    semantic_overlap=str(source.get("semantic_overlap") or ""),
                    material_difference=str(source.get("material_difference") or ""),
                    novelty_conclusion=str(source.get("novelty_conclusion") or ""),
                )
            )
